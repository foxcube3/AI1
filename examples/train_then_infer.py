...
def _adam_update(
    W, b, dW, db, mW, vW, mb, vb, lr: float, beta1: float, beta2: float, eps: float, t: int
) -> None:
...
def _sgd_update(W, b, dW, db, lr: float) -> None:
    V, D = len(W), (len(W[0]) if W else 0)
    for v in range(V):
        for d in range(D):
            W[v][d] -= lr * dW[v][d]
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Train a next-token head and immediately run inference.")
    # Shared model/tokenizer args
    parser.add_argument("--corpus", type=str, default="allen.txt", help="Path to corpus text file.")
    parser.add_argument("--prompt", type=str, default="Allen allows", help="Prompt for inference after training.")
    ...
    parser.add_argument("--save_head", type=str, default="", help="Optional path to save trained head.")
    parser.add_argument("--top_k", type=int, default=10, help="Top-k predictions to show in inference.")
    ...
    encoder = TransformerEncoder(
        num_layers=args.layers, dim=args.dim, num_heads=args.heads, ff_hidden=args.ff, seed=args.seed
    )
    ...
    if not prompt_tokens:
        ...
        return
    X = emb.embed_tokens(prompt_tokens)
    if pe is not None:
        X = pe.add_to(X, offset=0)
    ...
    topk = sorted(range(vocab_size), key=lambda i: probs[i], reverse=True)[: max(1, args.top_k)]
